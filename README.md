...
- `launch_llm_review_cycle.py` — wrapper graph generator for `llm_client review-cycle` tasks.
...
| `review_cycle` | `llm_review_cycle` | Delegates review/apply semantics to `llm_client review-cycle` and links the artifact sidecar |
...
**Acceptance semantics for llm_review_cycle tasks:**
1. OpenClaw invokes `python -m llm_client review-cycle --task-file <task.json>`
2. `llm_client` owns review/apply/review semantics and writes its run artifacts
3. OpenClaw writes `review_cycle_artifacts.json` as a sidecar index and links
   that sidecar plus `signoff.json` from the task report
4. OpenClaw does not replace graph/flat task report schemas with
   `AdversarialReview`
...
- Graph reports include bounded `task_results` summaries plus `planner_lineage`, `review_gate`, and `commit_evidence` fields when generated by the planner.
- Graph reports for `delivery_mode: llm_review_cycle` include a
  `review_cycle_artifacts` reference to the sidecar and signoff written by the
  delegated `llm_client review-cycle` run.
