...
.PHONY: review-gate-log cost errors summary

review-gate-log:  ## Show review-cycle gate outcomes (DAYS=30)
	@python scripts/review_gate_log.py --days $(or $(DAYS),30)

cost:  ## Show LLM cost breakdown by task (DAYS=7)
	@python scripts/task_cost.py --days $(DAYS)

errors:  ## Show recent task failures with failure reason (DAYS=7)
	@python scripts/task_errors.py --days $(DAYS)

summary:  ## Show task success/fail/gate/commit rates (DAYS=7)
	@python scripts/task_summary.py --days $(DAYS)
